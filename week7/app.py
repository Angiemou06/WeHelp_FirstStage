...
def apiMember():
    try:
        if "user" in session:
            ...
            con, cursor = connect_to_database()
            cursor.execute("SELECT id,name,username FROM member WHERE username=%s",(member_username,))
            result = cursor.fetchone()
            if result is not None:
                data_member_id = result[0]
                data_member_name=result[1]
                data_member_username=result[2]
                con.close()
                data = {
                    "id":data_member_id,
                    "name":data_member_name,
                    "username":data_member_username
                }
